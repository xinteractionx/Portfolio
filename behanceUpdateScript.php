 ...
 * https://github.com/behance/network_api_php
 *
...
/**
 * adding the call directly into library file to make deployment on server easier
 * To run this locally, type on a console
 * > php path/to/behanceUpdateScript.php
 */ 
try {
	$api = new Be_Api( '351', '9bv4hbQseJ258JwsJOkxssao0bGubDtM');
	$filePath = __DIR__ . '/projects.json';
	$projects = array();
	$api->getUserProjects( 'freysang' );
	foreach ($api->getUserProjects( 'freysang' ) as $project) {
		$projects[] = $api->getProject($project->id); 
	}
	if (count($projects) > 0) {
		file_put_contents($filePath, json_encode($projects));
		//echo "successfully wrote " . count($projects) . " projects file\n";
	} else {
		echo "something went wrong, we didn't get any projects but no error messages either\n";
	}
} catch (Exception $e) {
	$message = $e->getMessage();
	if ($message == 'Unsuccessful Request, response (429): '){
		echo "ran out of requests per hour, we just have to wait\n";
	} else {
		echo "unexpected error happened:" . $message ."\n";
	}
}